# Cerebra (CRB) — Архитектура проекта

> Полное техническое описание криптовалюты Cerebra: алгоритм, блокчейн, нода,
...
Cerebra — это Proof-of-Work-криптовалюта, написанная с нуля на Go (только
...
├── cmd/
│   ├── cerebrad/main.go        // демон полной ноды
│   ├── cerebra-miner/main.go   // отдельный CPU-майнер (Win/Linux)
│   └── cerebra-wallet/main.go  // CLI-кошелёк
├── web/
│   ├── site/index.html         // лендинг проекта + мини-эксплорер
│   └── wallet/index.html       // веб-кошелёк (ключи только в браузере)
├── deploy/
│   └── cerebrad.service        // systemd-юнит seed-ноды
└── tools/
...
заголовком `X-Cerebra-Peer`; список сохраняется в `peers.json` (лимит 64).
...
### 5.5. Флаги `cerebrad`
```
-datadir   каталог данных (по умолч. cerebra-data)
...
## 6. Отдельный майнер (`cmd/cerebra-miner`)
...
Флаги:
```
-node     базовый URL RPC ноды (по умолч. http://188.34.181.191/cerebra/api)
...
Запуск:
```
cerebra-miner-windows-amd64.exe -addr crb1ВАШ_АДРЕС
cerebra-miner-windows-amd64.exe -addr crb1ВАШ_АДРЕС -threads 4
```
...
## 7. CLI-кошелёк (`cmd/cerebra-wallet`)

```
cerebra-wallet -new                          # создать пару ключей (печатает приватный ключ + адрес)
cerebra-wallet -balance crb1...              # баланс адреса
cerebra-wallet -key <128hex> -send crb1... -amount 12.5 [-fee 0.0001]
cerebra-wallet -node http://.../api ...      # выбрать ноду
```
...
Доступ: `http://188.34.181.191/cerebra/wallet/`.
...
Доступ: `http://188.34.181.191/cerebra/`.
...
```
/opt/cerebra/src         исходники
/opt/cerebra/release     собранные бинарники (linux+windows)
/opt/cerebra/bin         рабочие бинарники ноды
/opt/cerebra/NETWORK_WALLET.txt   ключ кошелька seed-ноды (chmod 600)
/var/lib/cerebra         данные цепи (blocks.jsonl, peers.json)
/var/www/html/cerebra/   сайт, /wallet/, /downloads/
/usr/local/go            Go 1.22 для сборки
```

### 10.2. systemd (`deploy/cerebrad.service`)
Юнит `cerebrad`: запускает ноду с `-datadir /var/lib/cerebra -p2p :18750
...
соседям). Управление: `systemctl {status|restart|stop} cerebrad`,
логи — `journalctl -u cerebrad`.

### 10.3. Apache (reverse proxy)
Конфиг `/etc/apache2/conf-available/cerebra.conf`:
```
ProxyPass        /cerebra/api/ http://127.0.0.1:18751/api/
ProxyPassReverse /cerebra/api/ http://127.0.0.1:18751/api/
```
RPC слушает только localhost; наружу проброшен через Apache по пути
`/cerebra/api/`. Сайт и кошелёк — статика в `/var/www/html/cerebra/`.
...
### 10.5. Сборка релизов
Кросс-компиляция из `/opt/cerebra/src`:
```
GOOS={linux,windows} GOARCH=amd64 go build -ldflags '-s -w' -o <out> ./cmd/<app>
```
Выкладываются в `/var/www/html/cerebra/downloads/`:
`cerebrad`, `cerebra-miner`, `cerebra-wallet` (linux-amd64 и windows-amd64.exe)
...
- **`KEYS.txt`** (локально) и **`/opt/cerebra/NETWORK_WALLET.txt`** (сервер,
...
- Импорт в веб-кошелёк: `/cerebra/wallet/` → «Импортировать ключ» → вставить
  приватный ключ.
...
| Что                | Адрес                                                |
|--------------------|------------------------------------------------------|
| Сайт               | http://188.34.181.191/cerebra/                       |
| Веб-кошелёк        | http://188.34.181.191/cerebra/wallet/                |
| API статус         | http://188.34.181.191/cerebra/api/status             |
| Скачать майнер/ноду| http://188.34.181.191/cerebra/downloads/             |
| Seed-нода (P2P)    | http://188.34.181.191:18750                          |
| Исходники          | http://188.34.181.191/cerebra/downloads/cerebra-src.tar.gz |
...
Cerebra — экспериментальное ПО, запущенное за один день, без премайна, фонда,

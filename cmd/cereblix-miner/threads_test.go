...
func TestRecommendedThreads(t *testing.T) {
	tests := []struct {
		...
		{name: "apple silicon keeps cpu count with c vm fast path", goos: "darwin", goarch: "arm64", cpus: 10, want: 10},
		{name: "apple silicon eight core keeps cpu count", goos: "darwin", goarch: "arm64", cpus: 8, want: 8},
